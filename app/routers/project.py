...
def create_project(new_project: CreateProject, 
    ...
    if not roles:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, 
                            detail="The project must have at least one role")

    if project_repository.get_project_by_user_id_and_title(current_user.id, new_project.name):
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, 
                            detail="The user already has a project named "+ new_project.name)

    if project_has_roles_with_same_name(roles):
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, 
                            detail="The project mustnt have two roles with the same name")
...
def update_project(project_id: int, updated_project: UpdateProject, 
                current_user: models.User = Depends(oauth2.get_current_user), 
                db: Session = Depends(get_db)):
    ...
    if updated_project.name != project.name:
        ...
        if project:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, 
                                detail="The user already has a project named " + project.name)
