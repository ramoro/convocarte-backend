...
def delete_academic_experience(academic_exp_id: int, db: Session = Depends(get_db)):
    academic_exp_repository = AcademicExperienceRepository(db)
    ...
    if not deleted:
        raise HTTPException(status_code=404, 
                            detail=f"Academic experience with id {academic_exp_id} not found")
    
    return Response(status_code=status.HTTP_204_NO_CONTENT)

@router.put("/{academic_exp_id}", response_model=AcademicExperienceBase)
def update_academic_experience(academic_exp_id: int, updated_academic_exp: AcademicExperienceUpdate,
                                db: Session = Depends(get_db)):
    academic_exp_repository = AcademicExperienceRepository(db)
