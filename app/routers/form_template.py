...
def create_form_template(form_template: CreateFormTemplate, 
    ...
    if form_template_repository.get_form_template_by_user_id_and_title(current_user.id, form_template.form_template_title):
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, 
                            detail="The user already has a form template with the title " + form_template.form_template_title)

    new_form_template = form_template_repository.add_new_form_template(current_user.id, 
                                                                       form_template.form_template_title, 
                                                                       form_template.form_template_fields)
    if not new_form_template:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, 
                            detail="An error occurred while creating the form template")
...
def get_form_template(form_template_id: int, db: Session = Depends(get_db)):
    ...
    if not form_template:
        raise HTTPException(status_code=404, 
                            detail=f"Form template with id {form_template_id} not found.")
...
def update_form_template(form_template: UpdateFormTemplate, current_user: models.User = 
    ...
    template_form = form_template_repository.get_form_template_by_user_id_and_title(current_user.id, 
                                                                                    form_template.form_template_title)
    ...
    if not updated_form:
        if error == "Not Found Error":
            raise HTTPException(status_code=404, 
                                detail=f"Form template with id {form_template.id} not found.")
        else:
            raise HTTPException(status_code=500, detail="Internal Server Error")
